...
TRANSLATED_CODE_GOES_HERE
  ...
  output = template.replace("INTENT_GOES_HERE", explanation).replace("TRANSLATED_CODE_GOES_HERE", translated_code)
  return output
...
gold_df = (
  ...
  .withColumn(
    "outputPath"
    ,f.concat_ws("/", f.lit(output_volume_path), f.col("loadDatetime"))
    )
