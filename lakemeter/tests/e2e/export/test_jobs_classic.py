"""E2E tests: Jobs Classic — API calculation vs Excel export verification.
...
Run: pytest tests/e2e/export/test_jobs_classic.py -v --timeout=600
"""
import json
import pytest
from tests.e2e.helpers.test_data import (
    ESTIMATE_CONFIGS, INSTANCE_TYPES, USAGE_RUN_BASED, USAGE_HOURLY, config_id,
)
...
def _generate_jobs_classic_params():
...
def jobs_classic_results():
    """Collect results across all tests in this module."""
    results = []
    yield results
    # Save results after all tests complete
    save_test_results(results, "test_results/e2e_jobs_classic.json")


class TestJobsClassicCalculation:
    """Verify Jobs Classic API calculation returns valid results."""

    @pytest.mark.parametrize(
        "cfg, driver, worker, photon, usage_label, usage",
        _generate_jobs_classic_params(),
    )
    def test_calculation_succeeds(self, e2e_client, cfg, driver, worker,
                                   photon, usage_label, usage, jobs_classic_results):
        result = e2e_client.calculate_jobs_classic(
...
class TestJobsClassicExcelExport:
    """Create estimates with Jobs Classic workloads, export to Excel,
    ...
    def test_export_run_based(self, e2e_client, cfg):
        """Test run-based Jobs Classic: create estimate, add line items, export, verify."""
        cloud, region, tier = cfg["cloud"], cfg["region"], cfg["tier"]
        instances = INSTANCE_TYPES[cloud]

        # Create estimate
        estimate = e2e_client.create_estimate(
            name=f"E2E-Jobs-Classic-RunBased-{config_id(cfg)}",
            cloud=cloud, region=region, tier=tier,
        )
        eid = estimate["estimate_id"]

        # Add line items (one per instance combo × photon)
        calc_results = []
        for i, (driver, worker) in enumerate(zip(instances["driver"], instances["worker"])):
            for photon in [False, True]:
                # Calculate via API
                api_result = e2e_client.calculate_jobs_classic(
                    cloud=cloud, region=region, tier=tier,
                    driver=driver, worker=worker, num_workers=4,
                ...
                e2e_client.add_line_item(eid, {
                    "workload_name": f"Jobs-{driver}-photon{photon}",
                    "workload_type": "JOBS",
    ...
    def test_export_hourly(self, e2e_client, cfg):
        """Test hourly Jobs Classic: create estimate, add line items, export, verify."""
        cloud, region, tier = cfg["cloud"], cfg["region"], cfg["tier"]
        instances = INSTANCE_TYPES[cloud]

        estimate = e2e_client.create_estimate(
            name=f"E2E-Jobs-Classic-Hourly-{config_id(cfg)}",
            cloud=cloud, region=region, tier=tier,
        )
        eid = estimate["estimate_id"]
        ...
        for i, (driver, worker) in enumerate(zip(instances["driver"], instances["worker"])):
            ...
            e2e_client.add_line_item(eid, {
                "workload_name": f"Jobs-Hourly-{driver}",
                "workload_type": "JOBS",
                "serverless_enabled": False,
                "driver_node_type": driver,
                "worker_node_type": worker,
