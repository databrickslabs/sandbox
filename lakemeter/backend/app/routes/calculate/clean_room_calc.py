"""Clean Room calculation endpoint.

Clean Room uses CLEAN_ROOMS_COLLABORATOR SKU.
Priced per collaborator per day (max 10 collaborators).
"""
import logging
...
from app.routes.calculate.schemas import CleanRoomCalculationRequest

logger = logging.getLogger(__name__)
router = APIRouter()


@router.post("/calculate/clean-room", tags=["Cost Calculation"])
def calculate_clean_room_cost(
    ...
    try:
        ...
        # Look up DBU price for Clean Room SKU
        ...
        dbu_per_day = float(collaborators)
        dbu_per_month = dbu_per_day * days_per_month
        ...
        response_data = {
            "success": True,
            "data": {
                "workload_type": "CLEAN_ROOM", "sku_type": sku_type,
                "configuration": {
                    "cloud": request.cloud.upper(), "region": request.region,
                    "tier": request.tier.upper(), "collaborators": collaborators,
                },
                "usage": {"days_per_month": days_per_month},
                "dbu_calculation": {
                    "dbu_per_collaborator_per_day": 1.0,
                    "dbu_per_day": dbu_per_day,
        ...
        logger.error(f"Error calculating Clean Room cost: {e}")
