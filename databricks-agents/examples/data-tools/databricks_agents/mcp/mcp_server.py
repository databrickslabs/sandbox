...
def setup_mcp_server(agent_app, config: Optional[MCPServerConfig] = None, fastapi_app=None):
    """
    Set up MCP server for an AgentApp.

    Args:
        agent_app: Object with agent_metadata attribute (AgentApp instance)
        config: Optional MCP server configuration
        fastapi_app: FastAPI app to add routes to. If None, uses agent_app
                     (backward compat for when AgentApp subclassed FastAPI).
    ...
    server.setup_routes(fastapi_app or agent_app)
