"""
Top-level CLI dispatcher for dbx-agent-app.

Commands:
    dbx-agent-app deploy    Deploy agents from agents.yaml
    dbx-agent-app validate  Validate agents.yaml without deploying
    dbx-agent-app status    Show deployment status
    dbx-agent-app destroy   Tear down deployed agents
    dbx-agent-app dashboard Launch the agent discovery dashboard locally
    dbx-agent-app platform  Deploy the Agent Platform as a Databricks App
"""
...
def _run_platform(args):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            pkg_version = _get_version("dbx-agent-app")
        except Exception:
            pkg_version = "0.3.0"
